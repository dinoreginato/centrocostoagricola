DROP FUNCTION IF EXISTS public.create_application_inventory(
  uuid, uuid, date, text, numeric, numeric, jsonb, boolean, numeric, numeric, text
);

DROP FUNCTION IF EXISTS public.update_application_inventory(
...
CREATE OR REPLACE FUNCTION public.create_application_inventory(
    ...
    p_fuel_machine_id uuid DEFAULT NULL
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    IF p_fuel_machine_id IS NOT NULL THEN
      SELECT m.company_id INTO v_machine_company_id
      FROM public.machines m
      WHERE m.id = p_fuel_machine_id;

      IF v_machine_company_id IS NULL OR v_machine_company_id <> v_company_id THEN
        RAISE EXCEPTION 'Máquina no pertenece a la empresa';
      END IF;
    END IF;
...
REVOKE ALL ON FUNCTION public.create_application_inventory(
  uuid, uuid, date, text, numeric, numeric, jsonb, boolean, numeric, numeric, text, uuid
) FROM PUBLIC;
...
REVOKE ALL ON FUNCTION public.update_application_inventory(
  uuid, uuid, uuid, date, text, numeric, numeric, jsonb, boolean, numeric, numeric, text, uuid
) FROM PUBLIC;
