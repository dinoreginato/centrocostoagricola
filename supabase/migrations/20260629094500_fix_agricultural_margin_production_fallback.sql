...
SELECT
  ...
  CASE
    WHEN COALESCE(pb.kg_produced_record, 0) > 0 THEN pb.kg_produced_record
    WHEN (COALESCE(ib.kg_sent_export, 0) + COALESCE(ib.kg_juice, 0)) > 0 THEN
      COALESCE(ib.kg_sent_export, 0) + COALESCE(ib.kg_juice, 0)
    ELSE 0
  END::numeric AS kg_produced,
  (COALESCE(pb.kg_produced_record, 0) > 0) AS has_production_record,
  ...
  CASE
    WHEN COALESCE(pb.kg_produced_record, 0) > 0 THEN 'production_records'
  ...
  CASE
    WHEN CASE
      ...
      ELSE 0
    END > 0 THEN
      COALESCE(cb.total_cost, 0) / NULLIF(
        CASE
          WHEN COALESCE(pb.kg_produced_record, 0) > 0 THEN pb.kg_produced_record
          WHEN (COALESCE(ib.kg_sent_export, 0) + COALESCE(ib.kg_juice, 0)) > 0 THEN
            COALESCE(ib.kg_sent_export, 0) + COALESCE(ib.kg_juice, 0)
