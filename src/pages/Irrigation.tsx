...
export const Irrigation: React.FC = () => {
  ...
  // Data State
  const [pendingItems, setPendingItems] = useState<IrrigationItem[]>([]);
      ...
      <div className="flex items-center justify-between">
        <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 flex items-center">
                <Droplets className="mr-2 h-8 w-8 text-blue-600" />
                Riego y Agua
            </h1>
            <p className="text-sm text-gray-500 dark:text-gray-400">Asigna costos de riego y electricidad a sectores</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-1 bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
            ...
            <div className="divide-y divide-gray-200 dark:divide-gray-700 max-h-[600px] overflow-y-auto">
                {pendingItems.length === 0 ? (
                    <div className="p-8 text-center text-gray-500 dark:text-gray-400 text-sm">No hay items de riego pendientes.</div>
                ) : (
                    pendingItems.map(item => (
                        <div 
                            key={item.id} 
                            onClick={() => handleSelectItem(item)}
                            className={`p-4 cursor-pointer hover:bg-blue-50 transition-colors ${selectedItemId === item.id ? 'bg-blue-50 ring-2 ring-inset ring-blue-500' : ''}`}
                        >
                    ...
                    {!editingAssignmentId && (
                        <div className="flex space-x-4 mb-6">
                            <button
                                ...
                                className={`flex-1 py-2 px-4 rounded-l-md text-sm font-medium border ${distributeBy === 'sector' ? 'bg-blue-50 border-blue-500 text-blue-700' : 'bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300'}`}
                            >
                                Por Sector Específico
                            </button>
                            <button
                                onClick={() => setDistributeBy('field')}
                                className={`flex-1 py-2 px-4 border text-sm font-medium border ${distributeBy === 'field' ? 'bg-blue-50 border-blue-500 text-blue-700' : 'bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300'}`}
                            >
                            ...
                            <button
                                ...
                                className={`flex-1 py-2 px-4 rounded-r-md text-sm font-medium border ${distributeBy === 'company' ? 'bg-blue-50 border-blue-500 text-blue-700' : 'bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300'}`}
                            >
                                Empresa General
                            </button>
                        </div>
                ...
                <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex flex-col md:flex-row justify-between items-center gap-4">
                    ...
                    <div className="flex items-center gap-2 w-full md:w-auto">
                        <input
                            ...
                            className="text-sm border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 flex-1 md:w-64"
                        />
                        <button
