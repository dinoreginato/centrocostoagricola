...
export const Dashboard: React.FC = () => {
  ...
  const syncAgrometRain = useCallback(async () => {
    ...
    companyFields.forEach((f: any) => {
      const fLat = f.latitude != null ? Number(f.latitude) : null;
      const fLon = f.longitude != null ? Number(f.longitude) : null;
      if (fLat != null && !Number.isNaN(fLat) && fLon != null && !Number.isNaN(fLon)) {
        locations.push({ kind: 'field', id: f.id, lat: fLat, lon: fLon });
      }
      (f.sectors || []).forEach((s: any) => {
        const sLat = s.latitude != null ? Number(s.latitude) : fLat;
        const sLon = s.longitude != null ? Number(s.longitude) : fLon;
        if (sLat != null && !Number.isNaN(sLat) && sLon != null && !Number.isNaN(sLon)) {
          locations.push({ kind: 'sector', id: s.id, lat: sLat, lon: sLon });
        }
      });
    });
    ...
    try {
      ...
      (existingLogs || []).forEach((l: any) => {
        ...
        if (!date) return;
        const keyBase = `${date}`;
        if (l.sector_id) existing.add(`sector:${l.sector_id}:${keyBase}`);
        if (l.field_id) existing.add(`field:${l.field_id}:${keyBase}`);
        if (!l.sector_id && !l.field_id) existing.add(`company:${selectedCompany.id}:${keyBase}`);
      });
      ...
      locations.forEach(loc => {
        ...
        Object.keys(stack).forEach(k => {
          ...
          if (!k.startsWith(String(activeYear))) return;
          const entry = stack[k] || {};
          const raw = entry['PP-SUM'];
          if (raw == null) return;
          const mm = Number(raw);
          if (Number.isNaN(mm)) return;

          const existingKey = `${loc.kind}:${loc.id}:${k}`;
          ...
          rowsToInsert.push({
            ...
            source: 'agrometeorologia',
            station_id: best.id,
            station_name: best.nombre,
            field_id: loc.kind === 'field' ? loc.id : null,
            sector_id: loc.kind === 'sector' ? loc.id : null
          });
        });
      });
  ...
  const scopedRainLogs = (rainLogs || []).filter((l: any) => {
    ...
    if (String(l.date).slice(0, 4) !== String(rainActiveYear)) return false;
    if (rainScope === 'company') return !l.field_id && !l.sector_id;
    if (rainScope === 'field') return l.field_id === rainFieldId;
    return l.sector_id === rainSectorId;
  });
                ...
                <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6 border border-gray-100 flex flex-col">
                    ...
                    <div className="flex flex-col gap-2 mb-4">
                        <div className="flex gap-2">
                            <select
                                value={rainScope}
                                onChange={(e) => setRainScope(e.target.value as any)}
                                className="w-40 text-sm border border-gray-200 bg-gray-50 rounded-lg p-2 focus:ring-blue-500 focus:border-blue-500 font-medium"
                            >
                            ...
                            <select
                                ...
                                onChange={(e) => {
                                    ...
                                    setRainFieldId(next);
                                    const f = companyFields.find((x: any) => x.id === next);
                                    const firstSector = f?.sectors?.[0]?.id || '';
                                    if (firstSector) setRainSectorId(firstSector);
                                }}
                                disabled={rainScope === 'company'}
                                className="flex-1 text-sm border border-gray-200 bg-gray-50 rounded-lg p-2 focus:ring-blue-500 focus:border-blue-500 font-medium disabled:opacity-50"
                            >
                                {companyFields.map((f: any) => (
                        ...
                        {rainScope === 'sector' && (
                            <select
                                value={rainSectorId}
                                onChange={(e) => setRainSectorId(e.target.value)}
                                className="w-full text-sm border border-gray-200 bg-gray-50 rounded-lg p-2 focus:ring-blue-500 focus:border-blue-500 font-medium"
                            >
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center bg-gray-50 p-3 rounded-lg">
                        ...
                        <span className="text-blue-700 text-xl font-black">{scopedRainTotal.toFixed(1)} mm</span>
                    </div>
                </div>
            </div>
